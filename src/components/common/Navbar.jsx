    ...
    <nav className='w-full bg-slate-800 absolute h-20 bg-transparent flex items-center justify-center shadow-lg'>
        <div className='w-9/12 mr-20 flex flex-row items-center justify-between'>
            ...
            <div className='flex items-center justify-center gap-4'>
                {
                    isLoggedin ?
                    (<button className=' bg-indigo-500 hover:bg-indigo-600 block font-bold text-white shadow-sm rounded-full px-4 py-2 duration-300 w-20' onClick={() => setIsLoggedin(false)}>Login</button>) :
                    (<button className=' bg-indigo-500 hover:bg-indigo-600 block font-bold text-white shadow-sm rounded-full px-4 py-2 duration-300 w-20' onClick={() => setIsLoggedin(true)}>Logout</button>)
                }
            </div>
        </div>
    </nav>
